...
from .dataset import Dataset, TensorBatchDataset, ListDataset
...
class NetworkWrapper(object):
    ...
    def _configure_layer(self, layer):
        with use_shape_wrapping(False):
            ...
            if (
                not self._ctx.builder_config.can_run_on_DLA(layer)
                and device_type == trt.DeviceType.DLA
            ):
    ...
    def _trt(self):
        if not hasattr(self, "_raw_trt"):
            ...
            self._raw_trt = ctx.network._network.add_constant(
                [1], np.array([_int(self)], dtype=np.int32)
            ).get_output(0)
