...
def convert_ConvTranspose2d(ctx):
    ...
    if output_padding[0] + output_padding[1] > 0:
        ...
        layer.post_padding = trt.tensorrt.DimsHW(
            padding[0] - output_padding[0], padding[1] - output_padding[1]
        )
    else:
...
@add_module_test(
    torch.float32, torch.device("cuda"), [(1, 3, 7, 7)], enabled=trt_version() < "7.0"
)
