...
def convert_ConvTranspose2d_trt7(ctx):
    ...
    assert module.dilation == 1 or all(
        [d == 1 for d in module.dilation]
...
@add_module_test(
    torch.float32, torch.device("cuda"), [(1, 10, 7, 7)], enabled=trt_version() >= "7.0"
)
...
@add_module_test(
    torch.float32, torch.device("cuda"), [(1, 10, 8, 8)], enabled=trt_version() >= "7.0"
)
...
@add_module_test(
    torch.float32, torch.device("cuda"), [(1, 10, 9, 9)], enabled=trt_version() >= "7.0"
)
