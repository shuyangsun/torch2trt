...
def get_interpolate_plugin(size, mode, align_corners):
    ...
    torch2trt_plugin = InterpolatePlugin(
...
@tensorrt_converter(
    ...
    enabled=trt_version() < "7.1" and has_interpolate_plugin(),
)
...
@add_module_test(
    torch.float32, torch.device("cuda"), [(1, 3, 1, 1)], enabled=trt_version() >= "7.1"
)
