...
def run(self, serialize=False):
    ...
    module_trt = torch2trt(
        module, inputs_conversion, max_workspace_size=1 << 20, **self.torch2trt_kwargs
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_onnx",
        help="Whether to test using ONNX or torch2trt tracing",
        action="store_true",
    )
    parser.add_argument(
        "--serialize",
        help="Whether to use serialization / deserialization of TRT modules before test",
        action="store_true",
    )
    ...
    for test in MODULE_TESTS:
        ...
        try:
            ...
            max_error, psnr_db, mse, fps, fps_trt, ms, ms_trt = run(
                test, serialize=args.serialize
            )

            # write entry
            line = (
                "| %70s | %s | %25s | %s | %.2E | %.2f | %.2E | %.3g | %.3g | %.3g | %.3g |"
                % (
                    name,
                    test.dtype.__repr__().split(".")[-1],
                    str(test.input_shapes),
                    str(test.torch2trt_kwargs),
                    max_error,
                    psnr_db,
                    mse,
                    fps,
            ...
            line = "| %s | %s | %s | %s | N/A | N/A | N/A | N/A | N/A |" % (
                name,
                test.dtype.__repr__().split(".")[-1],
                str(test.input_shapes),
                str(test.torch2trt_kwargs),
