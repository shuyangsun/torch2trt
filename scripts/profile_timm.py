...
class Args:
    ...
    log_level: Literal[
        "verbose", "info", "error", "warning", "internal_error"
    ] = "error"
    use_onnx: bool = False
                ...
                with redirect_stderr(ferr):
                    with redirect_stdout(fout):
                        with torch.no_grad():
                            ...
                            data = torch.randn(
                                self.batch_size, 3, self.size, self.size
                            ).cuda()
                            ...
                            if self.use_cached_engine and os.path.exists(
                                self.engine_filepath()
                            ):
                                print("Loading cached engine...")
                                model_trt = TRTModule()
                                model_trt.load_state_dict(
                                    torch.load(self.engine_filepath())
                                )
                            else:
                            ...
                            if self.save_engine:
                                ...
                                torch.save(
                                    model_trt.state_dict(), self.engine_filepath()
                                )
